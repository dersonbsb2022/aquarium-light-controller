...
class AquariumController:
    ...
    def compute_target(self) -> dict:
        ...
        interpolating smoothly between schedule points.
        """
        schedule = sorted(self.config["schedule"], key=lambda s: self._parse_time(s["time"]))
        if not schedule:
            return {"blue": 0, "white": 0, "uv": 0}

        now = self._now_minutes()
        trans = self.config.get("transition_minutes", 30)

        # Before first schedule point or after last — all off
        first_time = self._parse_time(schedule[0]["time"])
        last_time = self._parse_time(schedule[-1]["time"])
        ...
        # Find which two points we're between (including transition ramp)
        for i in range(len(schedule)):
            target_time = self._parse_time(schedule[i]["time"])
            prev_levels = (
                {"blue": schedule[i-1]["blue"], "white": schedule[i-1]["white"], "uv": schedule[i-1]["uv"]}
                if i > 0
                else {"blue": 0, "white": 0, "uv": 0}
            )
            curr_levels = {"blue": schedule[i]["blue"], "white": schedule[i]["white"], "uv": schedule[i]["uv"]}

            transition_start = target_time - trans

            if now < target_time:
                if now >= transition_start:
                    # We're in the transition ramp
                    ...
                    # We're before this transition started — hold previous level
                    return prev_levels
...
class APIHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/api/state":
            ...
            # Return 24h preview of brightness curves (for the chart)
            preview = []
            original_schedule = controller.config["schedule"]
            trans = controller.config.get("transition_minutes", 30)
            for minute in range(0, 1440, 2):  # every 2 minutes
                # Temporarily override now
                target = self._compute_at_minute(minute, original_schedule, trans)
                preview.append({
                    "minute": minute,
    ...
    def _compute_at_minute(self, now: float, schedule: list, trans: float) -> dict:
        """Compute target at a specific minute (for preview)."""
        import math
        schedule = sorted(schedule, key=lambda s: _parse_time_static(s["time"]))
        if not schedule:
            return {"blue": 0, "white": 0, "uv": 0}

        first_time = _parse_time_static(schedule[0]["time"])
        last_time = _parse_time_static(schedule[-1]["time"])
        ...
        for i in range(len(schedule)):
            target_time = _parse_time_static(schedule[i]["time"])
            prev_levels = (
                {"blue": schedule[i-1]["blue"], "white": schedule[i-1]["white"], "uv": schedule[i-1]["uv"]}
                if i > 0
                else {"blue": 0, "white": 0, "uv": 0}
    ...
    def do_POST(self):
        if self.path == "/api/config":
            ...
            controller.bulb = None
            ok = controller.connect()
            self._send_json({"ok": ok, "status": controller.status})
        else:
            self._send_json({"error": "not found"}, 404)


def _parse_time_static(t: str) -> float:
    parts = t.strip().split(":")
    return int(parts[0]) * 60 + int(parts[1])
